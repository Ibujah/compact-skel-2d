...
use crate::geometry::geometry_objects::Sphere;

pub struct SkeletonPersp {
    ...
    pub skel_spheres: HashMap<usize, Sphere>,
    pub covered: HashMap<usize, Vec<usize>>,

    pub final_skeleton: Vec<bool>,
}

pub fn persp_point(coords: [f32; 2], fu: f32, fv: f32, cu: f32, cv: f32) -> [f32; 3] {
    let x = (coords[0] - cu) / fu;
    let y = (coords[1] - cv) / fv;
    let z = 1. as f32;
...
pub fn persp_point_vec(pt: &Vector2<f32>, fu: f32, fv: f32, cu: f32, cv: f32) -> Vector3<f32> {
    let [x, y, z] = persp_point([pt[0], pt[1]], fu, fv, cu, cv);

    Vector3::new(x, y, z)
}

impl SkeletonPersp {
    pub fn new(fu: f32, fv: f32, cu: f32, cv: f32) -> SkeletonPersp {
        SkeletonPersp {
    ...
    pub fn get_non_covered_neighbors(
        ...
        let all_nei = self.get_neighbors(ind_triangle);
        let ind_tri = self.delaunay_triangles[ind_triangle];
        let cir = self.get_sphere(ind_triangle)?;
        let mut kept_nei = [None, None, None];
        self.covered
                ...
                Some(val) => {
                    ...
                    loop {
                        ...
                        if self.next_neighbor[icur] == i1 {
                            ...
                            let pt = persp_point_vec(
                                &self.boundary_points[icur],
                                self.fu,
                                self.fv,
                                self.cu,
                                self.cv,
                            ...
                            if dist < cir.radius + epsilon * 2. / (self.fu + self.fv) {
    ...
    pub fn get_sphere(&mut self, ind_triangle: usize) -> Result<Sphere> {
        match self.skel_spheres.get(&ind_triangle) {
            Some(val) => Ok(*val),
            None => {
                let [ind1, ind2, ind3] = self.delaunay_triangles[ind_triangle];

                let pt1 = persp_point_vec(
                    &self.boundary_points[ind1],
                    self.fu,
                    self.fv,
                    self.cu,
                    self.cv,
                );
                let pt2 = persp_point_vec(
                    &self.boundary_points[ind2],
                    self.fu,
                    self.fv,
                    self.cu,
                    self.cv,
                );
                let pt3 = persp_point_vec(
                    &self.boundary_points[ind3],
                    self.fu,
                    self.fv,
                    self.cu,
                    self.cv,
                ...
                let res = Sphere::from(&pt1, &pt2, &pt3, &Vector3::new(0., 0., 0.))?;

                self.skel_spheres.insert(ind_triangle, res);
                Ok(res)
            }
        }
    }
...
pub fn append_and_find_first(
    ...
    let mut points: Vec<[f64; 3]> = skeleton
        .boundary_points
        .iter()
        .map(|v| {
            persp_point(
...
pub fn find_nearest_maximum(skeleton: &mut SkeletonPersp, ind_tetrahedron: usize) -> Result<usize> {
    let mut ind = ind_tetrahedron;
    let mut cir = skeleton.get_sphere(ind_tetrahedron)?;
    ...
    loop {
        ...
        (upd, ind, cir) = nei.iter().fold(
            Ok((false, ind, cir)),
            |cur: Result<(bool, usize, Sphere)>, n| {
                let (upd, cur_ind, cur_cir) = cur?;
                match n {
                    Some(i) => {
                        let cir = skeleton.get_sphere(*i)?;
...
pub fn export_skel_txt(
    ...
    for i in 0..skel.final_skeleton.len() {
        if skel.final_skeleton[i] {
            let circle = skel.skel_spheres.get(&i).unwrap();
            writeln!(
                file_nod,
                "{} {} {}",

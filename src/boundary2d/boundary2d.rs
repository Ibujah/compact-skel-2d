...
impl Boundary2d {
    pub fn from_naive_algorithm(mask: &image::GrayImage) -> Result<Boundary2d> {
        ...
        for r_u32 in 0..(mask.height() + 1) {
            for c_u32 in 0..(mask.width() + 1) {
                ...
                let val00 = (if r_u32 != 0 && c_u32 != 0 {
                    mask_cc.get_pixel(c_u32 - 1, r_u32 - 1).0[0]
                } else {
                    0
                }) as i32;
                let val01 = (if r_u32 != 0 && c_u32 < mask_cc.width() {
                    mask_cc.get_pixel(c_u32, r_u32 - 1).0[0]
                } else {
                    0
                }) as i32;
                let val10 = (if r_u32 < mask_cc.height() && c_u32 != 0 {
                    mask_cc.get_pixel(c_u32 - 1, r_u32).0[0]
                } else {
                    0
                }) as i32;
                let val11 = (if r_u32 < mask_cc.height() && c_u32 < mask_cc.width() {
                    mask_cc.get_pixel(c_u32, r_u32).0[0]
                } else {
                    0
                }) as i32;
                ...
                if (val00 == val11 && !v01 && !v10) || (val10 == val01 && !v00 && !v11) {
                    return Err(anyhow::Error::msg(
                        "Adjacent corners not handled by naive boundary algorithm",
                    ));
                }
        ...
        for [n1, n2] in edgs {
            ...
            match (p1, p2) {
                (Some(&i1), Some(&i2)) => {
                    next_neighbor_wrk[i2] = i1;
                    prev_neighbor_wrk[i1] = i2;
                }
    ...
    pub fn from_marching_squares(mask: &image::GrayImage) -> Result<Vec<Boundary2d>> {
        ...
        fn insert_and_connect(
            map_marching_data: &mut HashMap<usize, MarchingData>,
            lin_len: i32,
            row: i32,
            col: i32,
            ind_cc: usize,
            ...
            match map_marching_data.get_mut(&ind_cc) {
                Some(map_cc) => {
                    map_cc
                        .points_march
                        .insert(inds[i0], Vector2::<f32>::new(pts[i0][0], pts[i0][1]));
                    map_cc
                        .points_march
                        .insert(inds[i1], Vector2::<f32>::new(pts[i1][0], pts[i1][1]));
        ...
        for r_u32 in 0..(mask.height() + 1) {
            for c_u32 in 0..(mask.width() + 1) {
                ...
                let val01 = (if r_u32 < mask_cc.height() && c_u32 != 0 {
                    mask_cc.get_pixel(c_u32 - 1, r_u32).0[0]
                } else {
                    0
                }) as usize;
                let val10 = (if r_u32 != 0 && c_u32 < mask_cc.width() {
                    mask_cc.get_pixel(c_u32, r_u32 - 1).0[0]
                } else {
                    0
                }) as usize;
        ...
        for (_, marching_data) in map_marching_data {
            ...
            let mut points_wrk = Vec::new();
            marching_data.points_march.iter().enumerate().fold(
                (&mut inds, &mut points_wrk),
    ...
    pub fn get_loops_pts(&self) -> Vec<Vec<Vector2<f32>>> {
        self.get_loops_ind()
            .into_iter()
            .map(|v| v.into_iter().map(|ind| self.points[ind]).collect())
            .collect()
    }
}
